...
REQUIRED_DATABASE_PROPERTIES = [
    PORTAL_PROPERTY_SPRING_DATABASE_USER,
    PORTAL_PROPERTY_SPRING_DATABASE_PW,
    PORTAL_PROPERTY_SPRING_DATABASE_URL,
]
...
class ValidationMessageFormatter(logging.Formatter):
    ...
    def format(self, record, *args, **kwargs):
        ...
        if (
            ...
            or hasattr(record, "line_number_list")
            or hasattr(record, "column_number")
            or hasattr(record, "column_number_list")
        ) and not hasattr(record, "filename_"):
            raise ValueError("Tried to log about a line/column with no filename")
        return super(ValidationMessageFormatter, self).format(record, *args, **kwargs)
    ...
    def format_aggregated(
        ...
        if attr_val is not None:
            ...
            raise ValueError(
                "Tried to format an absent non-optional log field: '%s'" % field_name
            )
...
class LogfileStyleFormatter(ValidationMessageFormatter):
    ...
    def format(self, record):
        ...
        if not hasattr(record, "filename_"):
            ...
            record.file_indicator = os.path.relpath(
                record.filename_.strip(), self.study_dir
            )
        record.line_indicator = self.format_aggregated(
            record, "line_number", " line %d:", " lines [%s]:", optional=True
        )
        record.column_indicator = self.format_aggregated(
            record, "column_number", " column %d:", " columns [%s]:", optional=True
        )
        ...
        if (
            self.previous_filename is not None
            and current_filename != self.previous_filename
        ):
            formatted_result = "\n" + formatted_result
...
class CollapsingLogMessageHandler(logging.handlers.MemoryHandler):
        ...
        for record_list in list(grouping_dict.values()):
            ...
            for field_name in record_list[0].__dict__:
                ...
                field_values = OrderedDict(
                    (record.__dict__[field_name], None) for record in record_list
                )
                ...
                if len(field_values) == 1:
                    ...
                    aggregated_field_dict[field_name + "_list"] = list(
                        field_values.keys()
                    )

            # add a new log record with these fields tot the output buffer
            aggregated_buffer.append(logging.makeLogRecord(aggregated_field_dict))
    ...
    def shouldFlush(self, record):
        ...
        return record.levelno == logging.DEBUG or super(
            CollapsingLogMessageHandler, self
        ).shouldFlush(record)
...
def get_meta_file_type(meta_dictionary, logger, filename):
    ...
    if "genetic_alteration_type" in meta_dictionary and "datatype" in meta_dictionary:
        ...
        "cancer_study_identifier" in meta_dictionary
        and "type_of_cancer" in meta_dictionary
    ):
        ...
        "cancer_study_identifier" in meta_dictionary
        and "resource_type" in meta_dictionary
    ):
        ...
        logger.error(
            "Could not determine the file type. Did not find expected meta file fields. Please check your meta files for correct configuration.",
...
def validate_types_and_id(meta_dictionary, logger, filename):
    ...
    if (
        "genetic_alteration_type" in meta_dictionary
        and "datatype" in meta_dictionary
        and "stable_id" in meta_dictionary
    ):
        alt_type_datatype_and_stable_id = {}
        script_dir = os.path.dirname(__file__)
        allowed_data_types_file_name = os.path.join(
            script_dir, "allowed_data_types.txt"
        )
        data_line_nr = 0
            ...
            for line in allowed_data_types_file:
                ...
                if data_line_nr > 1:
                    ...
                    if (
                        ...
                        data_type,
                    ) not in alt_type_datatype_and_stable_id:
                        alt_type_datatype_and_stable_id[
        ...
        if (genetic_alteration_type, data_type) not in alt_type_datatype_and_stable_id:
            ...
            raise RuntimeError(
                "Unexpected error: genetic_alteration_type and data_type combination not found in allowed_data_types.txt.",
                genetic_alteration_type,
                data_type,
            )
        ...
        # For these entries the stable_id is not validated, but assumed to be checked for uniqueness by the user.
            ...
            alt_type_datatype_and_stable_id[(genetic_alteration_type, data_type)][0]
            == "*"
        ):
            pass
        # validate stable_id:
            ...
            stable_id
            not in alt_type_datatype_and_stable_id[(genetic_alteration_type, data_type)]
        ):
            logger.error(
                "Invalid stable id for genetic_alteration_type '%s', "
...
def parse_metadata_file(
    filename, logger, study_id=None, case_list=False, gene_panel_list=None
):
        ...
        for line_index, line in enumerate(metafile):
            ...
            if ":" not in line:
                logger.error(
                    ...
                    extra={"filename_": filename, "line_number": line_index + 1},
                )
    ...
    stable_id_mandatory = META_FIELD_MAP[meta_file_type].get("stable_id", False)
        ...
        if field not in META_FIELD_MAP[meta_file_type]:
            ...
            if field == "meta_file_type":
                ...
                logger.warning(
                    ...
                    {True: "case list", False: "meta"}[case_list],
                    extra={"filename_": filename, "cause": field},
                )

    # check that cancer study identifiers across files so far are consistent.
    if (
        study_id is not None
        and "cancer_study_identifier" in meta_dictionary
        and study_id != meta_dictionary["cancer_study_identifier"]
    ):
        logger.error(
            "Cancer study identifier is not consistent across " "files, expected '%s'",
            study_id,
            extra={
                "filename_": filename,
    ...
    meta_study_attribute_size_dict = {
        "cancer_study_identifier": 255,
        "type_of_cancer": 63,
        "name": 255,
        "description": 1024,
        "citation": 200,
    ...
    if meta_file_type == MetaFileTypes.STUDY:
        for attribute in meta_study_attribute_size_dict:
            if attribute in meta_dictionary:
                if (
                    len(meta_dictionary[attribute])
                    > meta_study_attribute_size_dict[attribute]
                ):
                    logger.error(
                        "The maximum length of the '%s' "
                        "value is %s"
                        % (attribute, meta_study_attribute_size_dict[attribute]),
                        extra={
                            "filename_": filename,
                            "cause": meta_dictionary[attribute]
                            + " (%s)" % len(meta_dictionary[attribute]),
                        },
                    )
    ...
    if meta_file_type in (MetaFileTypes.CNA_CONTINUOUS, MetaFileTypes.CNA_LOG2):
        if meta_dictionary["show_profile_in_analysis_tab"] != "false":
            logger.error(
                ...
                extra={
                    "filename_": filename,
                    "cause": "show_profile_in_analysis_tab: %s"
                    % meta_dictionary["show_profile_in_analysis_tab"],
                },
            )
    ...
    if meta_file_type in (MetaFileTypes.SEG, MetaFileTypes.GISTIC_GENES):
        ...
        if (
            meta_dictionary["reference_genome_id"]
            not in valid_segment_reference_genomes
        ):
            logger.error(
    ...
    if meta_file_type in [MetaFileTypes.MUTATION, MetaFileTypes.MUTATION_UNCALLED]:
        if "swissprot_identifier" in meta_dictionary and meta_dictionary[
            "swissprot_identifier"
        ] not in ("name", "accession"):
            logger.error(
        ...
        if "gene_panel" in meta_dictionary:
            logger.warning(
                "Including the stable ID for gene panels in meta file might lead to incorrect "
    ...
    if gene_panel_list:
        ...
        if "gene_panel" in meta_dictionary:
            if (
                meta_dictionary["gene_panel"] not in gene_panel_list
                and meta_dictionary["gene_panel"] != "NA"
            ):
                logger.error(
        ...
        if "stable_id" in meta_dictionary:
            ...
            if meta_dictionary["datatype"] == "Z-SCORE":
                if "source_stable_id" in meta_dictionary:
                    expression_zscores_source_stable_ids[
                        meta_dictionary["source_stable_id"]
                    ] = filename
...
def run_java(*args):
    ...
    print(">", " ".join(full_cmd))
    process = Popen(full_cmd, stdout=PIPE, stderr=STDOUT, universal_newlines=True)
    ret = []
    ...
    if process.returncode == 64 or process.returncode == 2:
        raise RuntimeError(
            "Aborting. Step failed due to wrong parameters passed to subprocess."
        )
    # any other error:
...
def get_database_properties(properties_filename: str) -> Optional[PortalProperties]:
    ...
    for required_property in REQUIRED_DATABASE_PROPERTIES:
        if (
            required_property not in properties
            or len(properties[required_property]) == 0
        ):
    ...
    if (
        properties.get(PORTAL_PROPERTY_DATABASE_HOST) is not None
    ...
    return PortalProperties(
        properties[PORTAL_PROPERTY_SPRING_DATABASE_USER],
...
def parse_properties_file(properties_filename: str) -> Dict[str, str]:
    if not os.path.exists(properties_filename):
        print(
            "properties file %s cannot be found" % properties_filename, file=ERROR_FILE
        )
        sys.exit(2)
    ...
    try:
        ...
        if url_elements.query.get("useSSL") == "true":
            ...
            if url_elements.query.get("get-server-public-key") == "true":
                connection_kwargs["ssl"] = {"MYSQL_OPT_GET_SERVER_PUBLIC_KEY": True}
        connection = MySQLdb.connect(**connection_kwargs)
    except MySQLdb.Error as exception:
        print(exception, file=ERROR_FILE)
        message = (
            "--> Error connecting to server with URL: " + portal_properties.database_url
        )
