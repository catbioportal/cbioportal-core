...
class ErrorFileFormatter(cbioportal_common.ValidationMessageFormatter):
    ...
    def format(self, record):
        ...
        record.rel_filename = os.path.relpath(record.filename_, self.study_dir)
...
class LineMessageFilter(logging.Filter):
    ...
    def filter(self, record):
        return int(hasattr(record, "filename_") and hasattr(record, "line_number"))
...
class Validator(object):
    ...
    def __init__(
        ...
        self.logger = CombiningLoggerAdapter(logger, extra={"filename_": self.filename})
    ...
    def _validate_file(self):
        ...
        try:
            ...
            self.logger.error(
                "File contains invalid UTF-8 bytes. Please check values in file"
            )
            return
        ...
        with open(self.filename, "r", newline=None) as data_file:
            ...
            top_comments = []
            line_number = 0
            for line_number, line in enumerate(data_file, start=line_number + 1):
            ...
            # if the loop wasn't broken by a non-commented line
                ...
                self.logger.error(
                    "No column header or data found in file",
            ...
            if not self.processTopLines(top_comments):
                self.logger.error("Invalid header comments, file cannot be parsed")
                if not self.relaxed_mode:
                    return
                else:
                    self.logger.info(
                        "Ignoring missing or invalid header comments. "
            ...
            try:
                ...
                self.logger.warning(
                    "Could not detect a tab separator. Confirm that the file should be single-column"
            ...
            if dialect.quotechar == '"' and not (
                ...
                or '"' + dialect.delimiter in sample_content
            ):
                dialect.quoting = csv.QUOTE_NONE
            if not self._checkTsvDialect(dialect):
                self.logger.error("Invalid file format, file cannot be parsed")
                return

            # parse the first non-commented line as the tsv header
            header_cols = next(
                csv.reader(
                    [header_line], delimiter="\t", quoting=csv.QUOTE_NONE, strict=True
                )
            )
            ...
            for unique_col_name in self.UNIQUE_COLUMNS:
                ...
                if col_index > -1:
                    self.unique_col_data[
                        _get_column_index(header_cols, unique_col_name)
                    ] = []

            if self.checkHeader(header_cols) > 0:
                if not self.relaxed_mode:
                    self.logger.error("Invalid column header, file cannot be parsed")
                    return
                else:
                    self.logger.warning(
                        "Ignoring invalid column header. "
                        "Continuing with validation..."
                    )

            # read through the data lines of the file
            csvreader = csv.reader(
                itertools.chain(first_data_lines, data_file),
                delimiter="\t",
                quoting=csv.QUOTE_NONE,
                strict=True,
            )
            for line_number, fields in enumerate(csvreader, start=line_number + 1):
                self.line_number = line_number
                if all(x.strip() == "" for x in fields):
                    self.logger.error(
                        "Blank line", extra={"line_number": self.line_number}
                    )
    ...
    def onComplete(self):
        ...
        self.logger.info(
            "Read %d lines. " "Lines with warning: %d. Lines with error: %d",
            self.line_number,
    ...
    def checkLine(self, data):
        ...
        if line_col_count != self.numCols:
            self.logger.error(
                "Expected %d columns based on header, " "found %d",
                self.numCols,
                line_col_count,
                extra={"line_number": self.line_number},
            )
        ...
        for col_name in self.REQUIRED_HEADERS:
            if col_name not in self.cols:
                self.logger.error(
                    ...
                    extra={
                        "line_number": self.line_number,
                        "cause": ", ".join(self.cols[: len(self.REQUIRED_HEADERS)])
                        + ", (...)",
                    },
                )
                num_errors += 1
    ...
    def _checkOrderedRequiredColumns(self):
        ...
        for col_index, col_name in enumerate(self.REQUIRED_HEADERS):
            if col_index >= self.numCols:
                num_errors += 1
                self.logger.error(
                    "Invalid header: expected '%s' in column %d," " found end of line",
                    col_name,
                    col_index + 1,
                    extra={"line_number": self.line_number},
                )
    ...
    def _checkTsvDialect(self, dialect):
        ...
        if dialect.delimiter != "\t":
            self.logger.error(
                "Not a tab-delimited file",
                extra={"cause": "delimiters of type: %s" % repr(dialect.delimiter)},
            )
            return False
        if dialect.quoting != csv.QUOTE_NONE:
            self.logger.warning(
                "Found quotation marks around field(s) in the first rows of the file. "
                ...
                extra={
                    "cause": "quotation marks of type: [%s] "
    ...
    def _checkLineBreaks(self):
        ...
        if self.newlines not in ("\r\n", "\r", "\n"):
            self.logger.error(
                "No line breaks recognized in file",
    ...
    def checkGeneIdentification(
        self, gene_symbol=None, entrez_id=None, profile_type=None
    ):
            ...
            if self.portal.species == "human":
                ...
                if gene_symbol != "":
                    ...
                    if gene_symbol[0] in [
                        ...
                        if (
                            self.portal.hugo_entrez_map is not None
                            and self.portal.alias_entrez_map is not None
                        ):
                            if (
                                gene_symbol not in self.portal.hugo_entrez_map
                                and gene_symbol not in self.portal.alias_entrez_map
                            ):
                                self.logger.warning(
                                    "Hugo Symbol is not in gene or alias table and starts with a "
                        ...
                        # with a number
                            ...
                            self.logger.warning(
                                "Hugo Symbol should not start with a number.",
                                extra={
                                    "line_number": self.line_number,
                                    "cause": gene_symbol,
                                },
        ...
        if self.portal.hugo_entrez_map is None or self.portal.alias_entrez_map is None:
                ...
                if gene_symbol is not None:
                    if (
                        ...
                        self.logger.warning(
                            ...
                            extra={
                                "line_number": self.line_number,
                                "cause": "(%s, %s)" % (gene_symbol, entrez_id),
                            },
                        )
            else:
                self.logger.warning(
        ...
        # no Entrez gene id, only a gene symbol
            ...
            # a gene
            num_entrezs_for_hugo = len(self.portal.hugo_entrez_map.get(gene_symbol, []))
            num_entrezs_for_alias = len(
                self.portal.alias_entrez_map.get(gene_symbol, [])
            )
            if num_entrezs_for_hugo == 1:
                # set the value to be returned
                identified_entrez_id = str(self.portal.hugo_entrez_map[gene_symbol][0])
                # check if there are other *different* Entrez gene ids associated
                # with this gene symbol
                other_entrez_ids_in_aliases = [
                    x
                    for x in self.portal.alias_entrez_map.get(gene_symbol, [])
                    if x != identified_entrez_id
                ]
                ...
                self.logger.error(
                    ...
                    "/".join(
                        str(entrez)
                        for entrez in self.portal.hugo_entrez_map[gene_symbol]
                    ),
                    extra={"line_number": self.line_number, "cause": gene_symbol},
                )
            # no canonical symbol, but a single unambiguous alias
                ...
                # set the value to be returned
                identified_entrez_id = str(self.portal.alias_entrez_map[gene_symbol][0])
    ...
    def load_chromosome_lengths(reference_genome, logger):
        ...
        chrom_size_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "chromosome_sizes.json"
        )
        try:
            ...
            raise FileNotFoundError(
                "Could not open chromosome_sizes.json. "
                "If it does not exist you can download it using the script "
                "downloadChromosomeSizes.py."
            )

        logger.debug(
            "Retrieving chromosome lengths from '%s'", os.path.basename(chrom_size_file)
        )
        ...
        try:
            ...
            raise KeyError(
                "Could not load chromosome sizes for genome build %s. Expecting one of "
                '["hg19", "hg38", "mm10", "felCat9"]' % reference_genome
            )
...
class FeaturewiseFileValidator(Validator):
        ...
        if feature_id in self._feature_id_lines:
            self.logger.warning(
                ...
                extra={
                    ...
                    % (feature_id, self._feature_id_lines[feature_id]),
                },
            )
            return
    ...
    def parseFeatureColumns(self, nonsample_col_vals):
        ...
        raise NotImplementedError(
            "The {} class did not provide a method to "
            "validate values in sample columns.".format(self.__class__.__name__)
        )
    ...
    def _set_sample_ids_from_columns(self):
        ...
        if len(self.cols[self.num_nonsample_cols :]) == 0:
            self.logger.error(
                "No sample columns found", extra={"line_number": self.line_number}
            )
            num_errors += 1
    ...
    def checkIdInSamples(self):
        ...
        for index, sample_id in enumerate(self.sampleIds):
            if not self.checkSampleId(
                sample_id, column_number=self.num_nonsample_cols + index + 1
            ):
                num_errors += 1
            if " " in sample_id:
                self.logger.error(
                    "White space in SAMPLE_ID is not supported",
                    extra={"line_number": self.line_number, "cause": sample_id},
                )
                num_errors += 1
        return num_errors
...
class GenewiseFileValidator(FeaturewiseFileValidator):
    ...
    def checkHeader(self, cols):
        ...
        if "Hugo_Symbol" in self.sampleIds or "Entrez_Gene_Id" in self.sampleIds:
            self.logger.error(
                "Hugo_Symbol or Entrez_Gene_Id need to be placed before the "
                "sample ID columns of the file.",
                extra={"line_number": self.line_number},
            )
            num_errors += 1
        elif not (
            "Hugo_Symbol" in self.nonsample_cols
            or "Entrez_Gene_Id" in self.nonsample_cols
        ):
            self.logger.error(
                "At least one of the columns Hugo_Symbol or "
            ...
            self.logger.warning(
                "The recommended column Entrez_Gene_Id was not found. "
...
class ContinuousValuesValidator(GenewiseFileValidator):
    ...
    def checkValue(self, value, col_index):
        ...
        if stripped_value not in self.NULL_VALUES and not self.checkFloat(
            stripped_value
        ):
            self.logger.error(
                "Value is neither a real number nor " + ", ".join(self.NULL_VALUES),
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
...
class CNADiscreteValidator(CNAValidator):
    ...
    ALLOWED_VALUES = [
        ...
        "1",
        "2",
    ] + GenewiseFileValidator.NULL_VALUES
    ...
    def validate(self):
        ...
        if "pd_annotations_filename" in self.meta_dict:
            ...
            annotations_validator = CNADiscretePDAAnnotationsValidator(
                ...
                self.logger.logger,
                self.relaxed_mode,
                self.strict_maf_checks,
            )
            annotations_validator.data_entrez_gene_ids = set(
                self._feature_id_lines.keys()
            )
...
class CustomDriverAnnotationValidator(Validator):
    ...
    def checkHeader(self, cols):
        ...
        if "cbp_driver_annotation" in cols and "cbp_driver" not in cols:
            self.logger.error(
                "Column cbp_driver_annotation "
                "found without any cbp_driver "
                "column.",
                extra={"column_number": cols.index("cbp_driver_annotation")},
            )
            num_errors += 1
        if "cbp_driver_tiers_annotation" in cols and "cbp_driver_tiers" not in cols:
            self.logger.error(
                "Column cbp_driver_tiers_annotation "
                "found without any cbp_driver_tiers "
                "column.",
                extra={"column_number": cols.index("cbp_driver_tiers_annotation")},
            )
            num_errors += 1
    ...
    def checkLine(self, data):
        ...
        has_custom_driver_cols = (
            len(
                list(set(self.cols) & set(self.CUSTOM_DRIVER_CHECK_FUNCTION_MAP.keys()))
            )
            > 0
        )
        ...
        driver_value, driver_annotation, driver_tiers_value, driver_tiers_annotation = (
            self.parsePdAnnotations(data)
        )
        if driver_tiers_annotation is not None and driver_tiers_value is None:
            self.logger.error(
                ...
                extra={"line_number": self.line_number, "cause": driver_tiers_value},
            )
    ...
    def parsePdAnnotations(self, data):
        ...
        if "cbp_driver_tiers_annotation" in self.cols:
            driver_tiers_annotation = data[
                self.cols.index("cbp_driver_tiers_annotation")
            ].strip()
            # treat the empty string as a missing value
    ...
    def checkDriverTiers(self, value):
        ...
        if value not in self.NULL_DRIVER_TIERS_VALUES:
            self.logger.info(
                'Values contained in the column cbp_driver_tiers that will appear in the "Mutation Color" '
                "menu of the Oncoprint",
                extra={
                    "line_number": self.line_number,
                    "column_number": self.cols.index("cbp_driver_tiers"),
                    "cause": value,
                },
            )
        ...
        if len(self.tiers) > 10:
            self.logger.warning(
                "cbp_driver_tiers contains more than 10 different tiers.",
                extra={
                    "line_number": self.line_number,
                    "column_number": self.cols.index("cbp_driver_tiers"),
                    "cause": value,
    ...
    def printDataInvalidStatement(self, value, col_index):
        ...
        message = "Value in column '%s' is invalid" % self.cols[col_index]
...
class CustomNamespacesValidator(Validator):
    ...
    def checkHeader(self, cols):
        ...
        for namespace in namespaces:
            ...
            defined_namespace_found = any(
                [
                    True
                    for col in cols
                    if col.lower().startswith(defined_namespace + ".")
                ]
            )
            if not defined_namespace_found:
                self.logger.error(
                    "%s namespace defined but the file does not have any matching columns"
                    % (defined_namespace)
                )
                num_errors += 1
...
class CNADiscreteLongValidator(
    CustomDriverAnnotationValidator, CustomNamespacesValidator
):
    """Logic to validate discrete long CNA data."""

    REQUIRED_HEADERS = ["Sample_Id", "Value"]
    OPTIONAL_HEADERS = [
        "Hugo_Symbol",
        "Entrez_Gene_Id",
        "cbp_driver",
    ...
    def checkHeader(self, cols):
        ...
        if "Hugo_Symbol" not in cols and "Entrez_Gene_Id" not in cols:
            self.logger.error(
        ...
        if entrez_gene_id is not None:
            if sample_id in self.cna_entries.keys():
                if entrez_gene_id in self.cna_entries[sample_id]:
                    if (
                        hugo_symbol is not None
                        and self.cna_entries[sample_id][entrez_gene_id].hugo is not None
                    ):
                        if (
                        ...
                        hugo_symbol is not None
                        or self.cna_entries[sample_id][entrez_gene_id].hugo is not None
                    ):
                        ...
                        self.logger.error(
                            ...
                            extra={
                                ...
                                % (
                                    ...
                                    else self.cna_entries[sample_id][
                                        entrez_gene_id
                                    ].hugo,
                                    entrez_gene_id,
                    ...
                    self.cna_entries[sample_id][entrez_gene_id] = self.cna_entry(
                        hugo=hugo_symbol, line=self.line_number
                    )
            else:
                self.cna_entries[sample_id] = {
                    entrez_gene_id: self.cna_entry(
                        hugo=hugo_symbol, line=self.line_number
                    )
                }
...
class CNADiscretePDAAnnotationsValidator(CustomDriverAnnotationValidator):
    ...
    def onComplete(self):
        ...
        if self.data_entrez_gene_ids is not None:
            extra_entrez_gene_ids = self.entrez_gene_ids.difference(
                self.data_entrez_gene_ids
            )
...
class MutationsExtendedValidator(
    CustomDriverAnnotationValidator, CustomNamespacesValidator
):
    ...
    VARIANT_CLASSIFICATION_VALUES = (
        ...
        + SKIP_VARIANT_TYPES
        + EXTRA_VARIANT_CLASSIFICATION_VALUES
        + ["Unknown"]
    )
    ...
    def checkHeader(self, cols):
        ...
        elif "swissprot_identifier" not in self.meta_dict:
            self.logger.warning(
        ...
        if not ("HGVSp_Short" in cols or "Amino_Acid_Change" in cols):
            self.logger.error(
                "At least one of the columns HGVSp_Short or "
        ...
        if ascn_namespace_defined:
            ...
            if len(missing_ascn_columns) > 0:
                self.logger.error(
                    "ASCN namespace defined but MAF "
    ...
    def checkLine(self, data):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            try:
                ...
                if not checking_function(value):
                    ...
                    raise RuntimeError(
                        (
                            "Checking function %s set an error "
        ...
        if (
            normalized_gene
            and self.portal.gene_panel_list
            and data[sample_id_column_index] in sample_ids_panel_dict
        ):
            panel_id = sample_ids_panel_dict[data[sample_id_column_index]]
            if panel_id in self.portal.gene_panel_list and panel_id != "NA":
                self.checkOffPanelVariant(
                    data, normalized_gene, panel_id, hugo_symbol, entrez_id
                )

        # check if a non-blank amino acid change exists for non-splice sites
        if "Variant_Classification" not in self.cols or data[
            self.cols.index("Variant_Classification")
        ] not in ("Splice_Site",):
            aachange_value_found = False
            for aa_col in ("HGVSp_Short", "Amino_Acid_Change"):
                if (
                    aa_col in self.cols
                    and data[self.cols.index(aa_col)] not in self.NULL_AA_CHANGE_VALUES
                ):
    ...
    def checkAlleleMAFFormat(self, data):
        ...
        necessary_columns_check6 = [
            "Reference_Allele",
            "Tumor_Seq_Allele1",
            "Tumor_Seq_Allele2",
        ]
        necessary_columns_check10 = ["Start_Position", "End_Position"]
        necessary_columns_check11 = (
            ["Variant_Type"] + necessary_columns_check6 + necessary_columns_check10
        )
        ...
        if set(necessary_columns_check10).issubset(self.cols):
            ...
            if (
                data[self.cols.index("Start_Position")].strip() == ""
                or data[self.cols.index("Start_Position")] == "NA"
                or data[self.cols.index("End_Position")].strip() == ""
                or data[self.cols.index("End_Position")] == "NA"
            ):
                positions = False
            else:
                positions = True
                start_pos = float(
                    data[self.cols.index("Start_Position")].strip("'").strip('"')
                )
                end_pos = float(
                    data[self.cols.index("End_Position")].strip("'").strip('"')
                )
        ...
        if set(necessary_columns_check6).issubset(self.cols):
            ...
            illegal_values_ref = [
                value for value in list(ref_allele) if value not in legal_values
            ]
            illegal_values_tumor1 = [
                value for value in list(tumor_seq_allele1) if value not in legal_values
            ]
            illegal_values_tumor2 = [
                value for value in list(tumor_seq_allele2) if value not in legal_values
            ]

            # Else return an error message
            if (
                len(illegal_values_ref) > 0
                and len(illegal_values_tumor1) > 0
                and len(illegal_values_tumor2) > 0
            ):
                log_message = "All Allele Based columns contain invalid character."
                extra_dict = {
                    "line_number": self.line_number,
                    "cause": "(%s, %s, %s)"
                    % (ref_allele, tumor_seq_allele1, tumor_seq_allele2),
                }
                self.send_log_message(self.strict_maf_checks, log_message, extra_dict)
            elif len(illegal_values_ref) > 0:
                log_message = (
                    "Allele Based column Reference_Allele contains invalid character."
                )
                extra_dict = {"line_number": self.line_number, "cause": ref_allele}
                self.send_log_message(self.strict_maf_checks, log_message, extra_dict)
            elif len(illegal_values_tumor1) > 0:
                log_message = (
                    "Allele Based column Tumor_Seq_Allele1 contains invalid character."
                )
                extra_dict = {
                    "line_number": self.line_number,
                    "cause": tumor_seq_allele1,
                }
                self.send_log_message(self.strict_maf_checks, log_message, extra_dict)
            elif len(illegal_values_tumor2) > 0:
                log_message = (
                    "Allele Based column Tumor_Seq_Allele2 contains invalid character."
                )
                extra_dict = {
                    "line_number": self.line_number,
                    "cause": tumor_seq_allele2,
                }
                self.send_log_message(self.strict_maf_checks, log_message, extra_dict)
        ...
        if set(necessary_columns_check10).issubset(self.cols) and positions:
            if not (start_pos <= end_pos):
                log_message = (
                    "Start_Position should be smaller than or equal to End_Position."
                )
                extra_dict = {
                    "line_number": self.line_number,
                    "cause": "(%s, %s)" % (start_pos, end_pos),
                }
                self.send_log_message(self.strict_maf_checks, log_message, extra_dict)
        ...
        if set(necessary_columns_check11).issubset(self.cols):
            if variant_type == "INS":
                ...
                if positions:
                    if not (
                        ((end_pos - start_pos + 1) == len(ref_allele))
                        or ((end_pos - start_pos) == 1)
                    ):
                        log_message = (
                            "Variant_Type indicates insertion, but difference in Start_Position and "
                        ...
                        self.send_log_message(
                            self.strict_maf_checks, log_message, extra_dict
                        )
                if not (
                    (len(ref_allele) <= len(tumor_seq_allele1))
                    or (len(ref_allele) <= len(tumor_seq_allele2))
                ):
                    log_message = (
                        "Variant_Type indicates insertion, but length of Reference_Allele is bigger than "
            ...
            if variant_type == "DEL":
                ...
                if positions:
                    if not ((end_pos - start_pos + 1) == len(ref_allele)):
                        log_message = (
                            "Variant_Type indicates deletion, but the difference between Start_Position and "
                ...
                if len(ref_allele) < len(tumor_seq_allele1) or len(ref_allele) < len(
                    tumor_seq_allele2
                ):
                    log_message = (
                        "Variant_Type indicates deletion, but length of Reference_Allele is smaller than "
            ...
            if variant_type == "SNP":
                ...
                if not (
                    len(ref_allele) == 1
                    and len(tumor_seq_allele1) == 1
                    and len(tumor_seq_allele2) == 1
                ):
                    log_message = (
                        "Variant_Type indicates a SNP, but length of Reference_Allele, Tumor_Seq_Allele1 "
            ...
            if variant_type == "DNP":
                ...
                if not (
                    len(ref_allele) == 2
                    and len(tumor_seq_allele1) == 2
                    and len(tumor_seq_allele2) == 2
                ):
                    log_message = (
                        "Variant_Type indicates a DNP, but length of Reference_Allele, Tumor_Seq_Allele1 "
            ...
            if variant_type == "TNP":
                ...
                if not (
                    len(ref_allele) == 3
                    and len(tumor_seq_allele1) == 3
                    and len(tumor_seq_allele2) == 3
                ):
                    log_message = (
                        "Variant_Type indicates a TNP, but length of Reference_Allele, Tumor_Seq_Allele1 "
            ...
            if variant_type == "ONP":
                ...
                if (
                    len(ref_allele) != len(tumor_seq_allele1)
                    or len(tumor_seq_allele1) != len(tumor_seq_allele2)
                ) or (
                    len(ref_allele) <= 3
                    and len(tumor_seq_allele1) <= 3
                    and len(tumor_seq_allele2) <= 3
                    ...
                    log_message = (
                        "Variant_Type indicates a ONP, but length of Reference_Allele, "
            ...
            if (
                ...
                if (
                    ("-" in ref_allele)
                    or ("-" in tumor_seq_allele1)
                    or ("-" in tumor_seq_allele2)
                ):
                    log_message = (
                        "Variant_Type indicates a %s, but Reference_Allele, Tumor_Seq_Allele1 "
    ...
    def checkAlleleSpecialCases(self, data):
        ...
        if set(necessary_columns).issubset(self.cols):
            ...
            if (
                ref_allele == tumor_seq_allele1
                and tumor_seq_allele1 == tumor_seq_allele2
            ):
                log_message = (
                    "All Values in columns Reference_Allele, Tumor_Seq_Allele1 "
            ...
            if (
                variant_type == "DEL"
                and len(ref_allele) == len(tumor_seq_allele1)
                and len(ref_allele) == len(tumor_seq_allele2)
                and "-" not in tumor_seq_allele1
                and "-" not in tumor_seq_allele2
            ):
                log_message = (
                    "Variant_Type indicates a deletion, Allele based columns are the same length, "
    ...
    def checkValidationColumns(self, data):
        ...
        if set(necessary_columns_check7and8).issubset(self.cols):
            validation_status = (
                data[self.cols.index("Validation_Status")].strip().lower()
            )
            ...
            norm_allele1 = data[
                self.cols.index("Match_Norm_Validation_Allele1")
            ].strip()
            norm_allele2 = data[
                self.cols.index("Match_Norm_Validation_Allele2")
            ].strip()
        if set(necessary_columns_check13).issubset(self.cols):
            ...
            validation_method = (
                data[self.cols.index("Validation_Method")].strip().lower()
            )
        ...
        if set(necessary_columns_check7and8).issubset(self.cols):
            if validation_status == "valid" or validation_status == "invalid":
                ...
                illegal_values_tumor1 = [
                    value
                    for value in list(tumor_allele1)
                    if value not in legal_allele_values
                ]
                illegal_values_tumor2 = [
                    value
                    for value in list(tumor_allele2)
                    if value not in legal_allele_values
                ]
                illegal_values_norm1 = [
                    value
                    for value in list(norm_allele1)
                    if value not in legal_allele_values
                ]
                illegal_values_norm2 = [
                    value
                    for value in list(norm_allele2)
                    if value not in legal_allele_values
                ]
                ...
                if (
                    tumor_allele1 in ["", "NA"]
                    or tumor_allele2 in ["", "NA"]
                    or norm_allele1 in ["", "NA"]
                    or norm_allele2 in ["", "NA"]
                ):
                    log_message = (
                        "Validation Status is %s, but Validation Allele columns are empty."
                        % validation_status
                    )
                ...
                # Check when Allele Columns are not empty if the Allele Based columns contain either -,A,C,T,G
                    ...
                    len(illegal_values_tumor1) > 0
                    or len(illegal_values_tumor2) > 0
                    or len(illegal_values_norm1) > 0
                    or len(illegal_values_norm2) > 0
                ):
                    log_message = (
                        "At least one of the Validation Allele Based columns (Tumor_Validation_Allele1, "
                    ...
                    extra_dict = {
                        "line_number": self.line_number,
                        "cause": "(%s, %s, %s, %s)"
                        % (tumor_allele1, tumor_allele2, norm_allele1, norm_allele2),
                    }
                    self.send_log_message(
                        self.strict_maf_checks, log_message, extra_dict
                    )
                ...
                elif validation_status == "invalid" and (
                    tumor_allele1 != norm_allele1 or tumor_allele2 != norm_allele2
                ):
                    log_message = (
                        "When Validation_Status is invalid the Tumor_Validation_Allele "
        ...
        if set(necessary_columns_check13).issubset(self.cols):
            if validation_status == "valid" or validation_status == "invalid":
                if validation_method == "none" or validation_method == "na":
                    log_message = (
                        "Validation Status is %s, but Validation_Method is not defined."
                        % validation_status
                    )
                    extra_dict = {"line_number": self.line_number}
                    self.send_log_message(
        ...
        if set(necessary_columns_check9).issubset(self.cols):
            ...
            if mutation_status == "germline" and validation_status == "valid":
                if tumor_allele1 != norm_allele1 or tumor_allele2 != norm_allele2:
                    log_message = (
                        "When Validation_Status is valid and Mutation_Status is Germline, the "
            ...
            # to the reference alleles and the Tumor_Validation_Alleles should be different from the Reference_Allele
                ...
                if (norm_allele1 != norm_allele2 or norm_allele2 != ref_allele) and (
                    ...
                    log_message = (
                        "When Validation_Status is valid and Mutation_Status is Somatic, the "
    ...
    def checkOffPanelVariant(
        self, data, normalized_gene, panel_id, hugo_symbol, entrez_id
    ):
        try:
        ...
        if normalized_gene not in self.portal.gene_panel_list[panel_id]:
            if hugo_symbol:
                self.logger.warning(
                    ...
                    extra={"line_number": self.line_number, "cause": hugo_symbol},
                )
            else:
                self.logger.warning(
    ...
    def checkNCBIbuild(self, value):
        ...
        if value != "":
            ...
            if value != str(self.portal.ncbi_build):
                self.extra = (
                    "The reference genome in column NCBI_Build does not correspond with the "
                    "reference genome specified for this study (%s)"
                    % (self.portal.ncbi_build)
                )
                self.extra_exists = True
                return False
        return True
    ...
    def checkMatchedNormSampleBarcode(self, value):
        if value != "":
            if (
                "normal_samples_list" in self.meta_dict
                and self.meta_dict["normal_samples_list"] != ""
            ):
                normal_samples_list = [
                    x.strip() for x in self.meta_dict["normal_samples_list"].split(",")
                ]
                if value not in normal_samples_list:
                    self.extra = (
                        "Normal sample id not in list of sample ids configured in corresponding metafile. "
    ...
    def checkVerificationStatus(self, value):
        ...
        if self.checkNotBlank(value) and value.lower() not in (
            "verified",
            "unknown",
            "na",
        ):
    ...
    def checkAminoAcidChange(self, value):
        ...
        if value not in self.NULL_AA_CHANGE_VALUES:
            ...
            if len(value) > 1 and "p." in value[1:]:
                ...
                self.extra = (
                    "Unexpected 'p.' within amino acid change, "
            ...
            if ";" in value or "+" in value:
                # return with an error message
                self.extra = (
                    "Unexpected ';' or '+' in amino acid change, "
    ...
    def skipValidation(self, data):
        ...
        if "variant_classification_filter" in self.meta_dict:
            self.SKIP_VARIANT_TYPES = [
                x.strip()
                for x in self.meta_dict["variant_classification_filter"].split(",")
            ]
        ...
        if hugo_symbol == "Unknown" and entrez_id == "0":
            ...
            if variant_classification in ["IGR", "Targeted_Region"]:
                self.logger.info(
                    "This variant (Gene symbol 'Unknown', Entrez gene ID 0) will be filtered out",
                    extra={
                        "line_number": self.line_number,
            ...
            self.logger.info(
                "Line will not be loaded due to the variant "
    ...
    def checkSwissProt(self, value):
        ...
        if self.meta_dict.get("swissprot_identifier", "name") == "accession":
            if not re.match(
                ...
                self.logger.warning(
                    "SWISSPROT value is not a (single) UniProtKB accession. "
            ...
            if not re.match(r"^[A-Z0-9]{1,5}_[A-Z0-9]{1,5}$", value):
                # if there is a ',' then give a more detailed message:
                if "," in value:
                    self.logger.warning(
                        "SWISSPROT value is not a single UniProtKB/Swiss-Prot name. "
    ...
    def checkStartPosition(self, value):
        ...
        if value.strip() == "" or value == "NA":
            ...
            self.logger.error(
                "The start position of this variant is not " "an integer",
                extra={
                    "line_number": self.line_number,
                    "column_number": self.cols.index("Start_Position"),
                    "cause": value,
    ...
    def checkEndPosition(self, value):
        ...
        if value.strip() == "" or value == "NA":
            ...
            self.logger.error(
                "The end position of this variant is not " "an integer",
                extra={
                    "line_number": self.line_number,
                    "column_number": self.cols.index("End_Position"),
                    "cause": value,
    ...
    def checkMutationStatus(self, value):
        ...
        if value.lower() in ["loh", "none", "wildtype"]:
            self.logger.info(
                "Mutation will not be loaded due to value in Mutation_Status",
                extra={"line_number": self.line_number, "cause": value},
            )
        if (
            value.lower()
            ...
            self.logger.warning(
                "Mutation_Status value is not in MAF format",
                extra={"line_number": self.line_number, "cause": value},
            )
        return True
...
class ClinicalValidator(Validator):
    ...
    ALLOW_BLANKS = True
    METADATA_LINES = ("display_name", "description", "datatype", "priority")
    ...
    PREDEFINED_ATTRIBUTES = {
        "AGE": {"is_patient_attribute": "1", "datatype": "NUMBER"},
        "CANCER_TYPE": {"datatype": "STRING"},
        "CANCER_TYPE_DETAILED": {"datatype": "STRING"},
        "DFS_STATUS": {"is_patient_attribute": "1", "datatype": "STRING"},
        "DFS_MONTHS": {"is_patient_attribute": "1", "datatype": "NUMBER"},
        "DRIVER_MUTATIONS": {},
        "ERG_FUSION_ACGH": {},
        "ETS_RAF_SPINK1_STATUS": {},
        "GENDER": {"is_patient_attribute": "1", "datatype": "STRING"},
        "GLEASON_SCORE": {},
        "HISTOLOGY": {
            "datatype": "STRING",
        },
        ...
        "MOUSE_STRAIN": {},
        "OS_STATUS": {"is_patient_attribute": "1", "datatype": "STRING"},
        "OS_MONTHS": {"is_patient_attribute": "1", "datatype": "NUMBER"},
        "OTHER_SAMPLE_ID": {"is_patient_attribute": "0", "datatype": "STRING"},
        "PATIENT_DISPLAY_NAME": {"is_patient_attribute": "1", "datatype": "STRING"},
        "PRIMARY_SITE": {
            "is_patient_attribute": "0",
            "datatype": "STRING",
        },
        "SAMPLE_CLASS": {"is_patient_attribute": "0", "datatype": "STRING"},
        "SAMPLE_DISPLAY_NAME": {"is_patient_attribute": "0", "datatype": "STRING"},
        "SAMPLE_TYPE": {"is_patient_attribute": "0", "datatype": "STRING"},
        "SERUM_PSA": {},
        "SEX": {"is_patient_attribute": "1", "datatype": "STRING"},
        ...
        "TUMOR_TYPE": {"is_patient_attribute": "0", "datatype": "STRING"},
    }
    ...
    def processTopLines(self, line_list):
        ...
        if not line_list:
            if not self.relaxed_mode:
                ...
                self.logger.info(
                    "Ignoring missing or invalid data type definition "
        ...
        num_attrs = 0
        csvreader = csv.reader(
            line_list, delimiter="\t", quoting=csv.QUOTE_NONE, strict=True
        )
    ...
    def checkHeader(self, cols):
        ...
        if self.fill_in_attr_defs:
            self.logger.info(
                "Filling in missing attribute properties for clinical data."
            )
            missing_attr_defs = {}
            for col_index, col_name in enumerate(cols):
                missing_attr_defs[col_index] = {
            ...
            if col_name in self.PREDEFINED_ATTRIBUTES:
                for attr_property in self.PREDEFINED_ATTRIBUTES[col_name]:
                    if attr_property == "is_patient_attribute":
                        expected_level = self.PREDEFINED_ATTRIBUTES[col_name][
                            attr_property
                        ]
                    ...
                    if attr_property == "datatype":
                        ...
                        if not self.fill_in_attr_defs:
                            ...
                            expected_value = self.PREDEFINED_ATTRIBUTES[col_name][
                                attr_property
                            ]
                            if value != expected_value and not self.fill_in_attr_defs:
                                self.logger.error(
                                    ...
                                    expected_value,
                                    extra={
                                        "line_number": self.METADATA_LINES.index(
                                            attr_property
                                        )
                                        + 1,
                                        "column_number": col_index + 1,
    ...
    def checkLine(self, data):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if "date" not in col_name.lower():
                if "-" in value:
                    if len(value.split("-")) == 2 and not any(
                        i in value for i in [",", ".", ":", ";"]
                    ):
                        if any(
                            value_part.lower() in excel_months
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if col_name == "SAMPLE_ID":
                ...
                if len(value.strip()) > MAX_SAMPLE_STABLE_ID_LENGTH:
                    self.logger.error(
                        "SAMPLE_ID too long (%s character maximum)"
                        % (MAX_SAMPLE_STABLE_ID_LENGTH),
                        extra={
                            "line_number": self.line_number,
                            "column_number": col_index + 1,
                            "cause": value,
                    ...
                    if value.startswith("TCGA-"):
                        self.logger.warning(
                            ...
                            extra={
                                ...
                                "column_number": col_index + 1,
                                "cause": "%s (already defined on line %d)"
                                % (value, self.sample_id_lines[value]),
                            },
                        )
                    else:
                        self.logger.error(
    ...
    def checkLine(self, data):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if col_name == "PATIENT_ID":
                if value in self.patient_id_lines:
                    self.logger.error(
                        ...
                        extra={
                            ...
                            "cause": "%s (already defined on line %d)"
                            % (value, self.patient_id_lines[value]),
                        },
                    )
                else:
                    ...
                    if value not in PATIENTS_WITH_SAMPLES:
                        self.logger.warning(
                            "Clinical data defined for a patient with " "no samples",
                            extra={
                                "line_number": self.line_number,
                                "column_number": col_index + 1,
                                "cause": value,
                ...
                elif value.lower() not in self.NULL_VALUES and value not in (
                    "LIVING",
                    "DECEASED",
                    "0:LIVING",
                    "1:DECEASED",
                ...
                if value.lower() not in self.NULL_VALUES and value not in (
                    "0:DiseaseFree",
            ...
            # check other survival data
                ...
                if (survival_prefix + "_MONTHS") in self.cols:
                    ...
                    if (
                        len(splited_value) <= 1
                        or len(splited_value) >= 2
                        and splited_value[0] not in ("0", "1")
                    ):
        ...
        if osstatus_is_deceased and (
            osmonths_value is None or osmonths_value.lower() in self.NULL_VALUES
        ):
            ...
            self.logger.warning(
                ...
                extra={"line_number": self.line_number, "cause": osmonths_value},
            )
    ...
    def onComplete(self):
        ...
        for patient_id in PATIENTS_WITH_SAMPLES:
            if patient_id not in self.patient_id_lines:
                self.logger.warning(
                    "Missing clinical data for a patient associated with " "samples",
...
class SegValidator(Validator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.chromosome_lengths = self.load_chromosome_lengths(
            self.meta_dict["reference_genome_id"], self.logger.logger
        )
        # add 23 and 24 "chromosomes" as aliases to X and Y, respectively, unless its cat data
        if self.meta_dict["reference_genome_id"] != "felCat9":
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if col_name == "ID":
                ...
                if parsed_coords[col_name] < 0 or (
                    "chrom" in parsed_coords
                    and parsed_coords[col_name]
                    > self.chromosome_lengths[parsed_coords["chrom"]]
                ):
                    self.logger.error(
                        "Genomic position beyond end of chromosome " "(chr%s:0-%s)",
                        parsed_coords["chrom"],
                        self.chromosome_lengths[parsed_coords["chrom"]],
                        extra={
                            "line_number": self.line_number,
                ...
                raise RuntimeError("Could not validate column type: " + col_name)
...
class StructuralVariantValidator(CustomNamespacesValidator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.unique_fields = []
        self.other_required_genomic_fields = [
            "Site1_Region",
            "Site2_Region",
            "Site1_Contig",
            "Site2_Contig",
    ...
    def checkHeader(self, data):
        ...
        if not (
            "Site1_Hugo_Symbol" in self.cols or "Site1_Entrez_Gene_Id" in self.cols
        ):
            self.logger.error(
                "Structural variant event requires Site1_Entrez_Gene_Id and/or Site1_Hugo_Symbol column",
                extra={"line_number": self.line_number},
            )
            num_errors += 1
        ...
        if not (
            "Site2_Hugo_Symbol" in self.cols or "Site2_Entrez_Gene_Id" in self.cols
        ):
            self.logger.error(
                "Structural variant event requires Site2_Entrez_Gene_Id and/or Site2_Hugo_Symbol column",
                extra={"line_number": self.line_number},
            )
            num_errors += 1
        ...
        self.missing_genomic_fields.extend(
            [
                value
                for value in self.other_required_genomic_fields
        ...
        if len(self.missing_genomic_fields) > 1:
            self.logger.warning(
                "Missing genomic information. Consider" " adding the fields: [%s]",
                ", ".join(sorted(self.missing_genomic_fields)),
                extra={"line_number": self.line_number},
            )
    ...
    def checkLine(self, data):
        ...
        def checkNCBIbuild(ncbi_build):
            if ncbi_build is None:
                self.logger.warning(
                    "No value in NCBI_Build, assuming GRCh37 is the assembly",
                    extra={"line_number": self.line_number},
                )
            else:
                # Check NCBI build
                if ncbi_build.lower() not in ["grch37", "grch38"]:
                    self.logger.error(
                        "Only GRCh37 and GRCh38 are supported",
                        extra={"line_number": self.line_number, "cause": ncbi_build},
                    )
                ...
                if self.ncbi_build is None:
                    ...
                    if str(self.ncbi_build) != str(ncbi_build):
                        self.logger.error(
                            "Multiple values in NCBI_Build found, this is not supported",
                            extra={
                                "line_number": self.line_number,
        ...
        def checkSVstatus(sv_status):
            ...
            if sv_status is None:
                self.logger.warning(
                    "No value in SV_Status, assuming the variant is SOMATIC",
                    extra={"line_number": self.line_number},
                )
            else:
                if sv_status.lower() not in ["somatic", "germline"]:
                    self.logger.error(
                        "Invalid SV_Status value: possible values are [SOMATIC, GERMLINE]",
                        extra={"line_number": self.line_number, "cause": sv_status},
                    )
        ...
        if "NCBI_Build" in self.cols:
            ...
            self.logger.warning(
                "No column NCBI_Build, assuming GRCh37 is the assembly",
                extra={"line_number": self.line_number},
            )
        ...
        site1_gene = self.checkGeneIdentification(
            site1_hugo_symbol, site1_entrez_gene_id, "SV"
        )
        ...
        site2_gene = self.checkGeneIdentification(
            site2_hugo_symbol, site2_entrez_gene_id, "SV"
        )
        ...
        if site1_gene is None and site2_gene is None:
            ...
            if len(non_recognized_genes) > 0:
                self.logger.warning(
                    ...
                    extra={
                        "line_number": self.line_number,
                        "cause": ", ".join(non_recognized_genes),
                    },
                )
        ...
        sv_entry = "\t".join(
            [data[self.cols.index(attr)] for attr in self.unique_fields]
        )
        if sv_entry in self.sv_entries:
            self.logger.error(
                ...
                extra={
                    "line_number": self.line_number,
                    "cause": "%s (already defined on line %d)"
                    % (sv_entry, self.sv_entries[sv_entry]),
                },
            )
        else:
...
class GenePanelMatrixValidator(Validator):
    ...
    def checkHeader(self, data):
        ...
        for genetic_profile in study_meta_dictionary:
            ...
            if (
                study_meta_dictionary[genetic_profile]["genetic_alteration_type"]
                == "MUTATION_EXTENDED"
                and study_meta_dictionary[genetic_profile]["datatype"] == "MAF"
                and study_meta_dictionary[genetic_profile]["stable_id"] in data
            ):
            ...
            if "gene_panel" in study_meta_dictionary[genetic_profile]:
                property_in_meta_file_list.append(
                    study_meta_dictionary[genetic_profile]["stable_id"]
                )
        ...
        for stable_id in data:
            if stable_id != "SAMPLE_ID":
                if stable_id not in stable_ids:
                    self.logger.error(
                        "Stable ID not found in study meta files",
                ...
                if stable_id != mutation_stable_id:
                    if stable_id in property_in_meta_file_list:
                        self.logger.error(
                            "The meta file for this genetic profile contains a property for gene panel. "
                            ...
                            extra={"line_number": self.line_number, "cause": stable_id},
                        )
                    else:
                        self.logger.info(
                            "This column can be replaced by a 'gene_panel' property in the respective "
                            "meta file",
                            extra={"line_number": self.line_number, "cause": stable_id},
                        )
        return num_errors
    ...
    def checkLine(self, data):
        ...
        if sample_id in self.gene_panel_sample_ids:
            ...
            if self.mutation_stable_id_index is not None:
                sample_ids_panel_dict[sample_id] = data[
                    self.mutation_stable_id_index - 1
                ]
                ...
                if data[self.mutation_stable_id_index - 1] != "NA":
                    if sample_id not in mutation_sample_ids:
                        self.logger.error(
                            "Sample ID has mutation gene panel, but is not in the sequenced case list",
                            extra={"line_number": self.line_number, "cause": sample_id},
                        )
        ...
        if self.portal.gene_panel_list is not None:
            for gene_panel_id in data:
                if (
                    gene_panel_id not in self.portal.gene_panel_list
                    and gene_panel_id not in ["NA", "WGS", "WXS", "WXS/WGS"]
                ):
                    self.logger.error(
                        "Gene panel ID is not in database. Please import this gene panel before loading "
                        "study data.",
                        extra={"line_number": self.line_number, "cause": gene_panel_id},
                    )
...
class ProteinLevelValidator(FeaturewiseFileValidator):
    ...
    def parseFeatureColumns(self, nonsample_col_vals):
        ...
        if "|" not in value:
            self.logger.error(
                "No pipe symbol in Composite.Element.REF column",
                extra={
                    "line_number": self.line_number,
                    "column_number": 1,
...
class TimelineValidator(Validator):
    REQUIRED_HEADERS = ["PATIENT_ID", "START_DATE", "STOP_DATE", "EVENT_TYPE"]
    REQUIRE_COLUMN_ORDER = True
    ALLOW_BLANKS = True
...
class CancerTypeValidator(Validator):
    ...
    COLS = ("type_of_cancer", "name", "color", "parent_type_of_cancer")
    ...
    def checkLine(self, data):
        ...
        try:
            ...
            if len(data) != 4:
                self.logger.error(
                    "Lines in cancer type files must have these "
            ...
            for col_index, field_name in enumerate(self.cols):
                ...
                if value == "":
                    ...
                    # current study:
                    if (
                        parent_cancer_type != "tissue"
                        and self.portal.cancer_type_dict is not None
                        and not (
                            parent_cancer_type in self.portal.cancer_type_dict
                            or parent_cancer_type in self.defined_cancer_types
                        )
                    ):
                        self.logger.error(
            ...
            if line_cancer_type in self.defined_cancer_types:
                self.logger.error(
                    ...
                    extra={"line_number": self.line_number, "cause": line_cancer_type},
                )
            # compare the cancer_type definition with the portal instance
            if (
                self.portal.cancer_type_dict is not None
                and line_cancer_type in self.portal.cancer_type_dict
            ):
                ...
                for col_index, field_name in enumerate(self.cols):
                    ...
                    if field_name == "parent_type_of_cancer":
                        value = value.lower()
                    if (
                        field_name in existing_info
                        and value != existing_info[field_name]
                    ):
                        self.logger.error(
...
class ResourceDefinitionValidator(Validator):
    ...
    def checkLine(self, data):
        ...
        self.resource_definition_dictionary.setdefault(resource_id, []).append(
            resource_type
        )
...
class SampleResourceValidator(ResourceValidator):
    ...
    def checkLine(self, data):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if col_name == "RESOURCE_ID":
                if (
                    value not in RESOURCE_DEFINITION_DICTIONARY
                ...
                if (
                    value in RESOURCE_DEFINITION_DICTIONARY
            ...
            self.logger.error(
                ...
                extra={
                    ...
                    "duplicated_line_number": self.defined_resources[
                        (resource_id, sample_id, resource_url)
                    ],
                },
            )
        else:
            self.defined_resources[(resource_id, sample_id, resource_url)] = (
                self.line_number
            )
        ...
        if (resource_id, patient_id, resource_url) in self.defined_resources:
            self.logger.error(
                ...
                extra={
                    "line_number": self.line_number,
                    "duplicated_line_number": self.defined_resources[
                        (resource_id, patient_id, resource_url)
                    ],
                },
            )
        else:
            self.defined_resources[(resource_id, patient_id, resource_url)] = (
                self.line_number
            )
...
class GisticGenesValidator(Validator):
    ...
    def checkLine(self, data):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if (
                col_name in self.REQUIRED_HEADERS and col_name != "genes_in_region"
            ) and value.strip() in self.NULL_VALUES:
                self.logger.error(
                    "Empty cell in column '%s'",
                    col_name,
                    extra={
                        "line_number": self.line_number,
                        "column_number": col_index + 1,
            ...
            if col_name == "chromosome":
                ...
                self.parse_amp_value(value, column_number=col_index + 1)
            elif col_name == "cytoband":
                cytoband_chromosome, parsed_cytoband = self.parse_cytoband(
                    value, column_number=col_index + 1
                )
            elif col_name == "q_value":
                self.parse_q_value(value, column_number=col_index + 1)
        ...
        if parsed_peak_start is not None and parsed_peak_end is not None:
            if parsed_peak_start > parsed_peak_end:
                ...
                self.logger.error(
                    ...
                    extra={
                        "line_number": self.line_number,
                        "cause": "{}/{}".format(parsed_peak_start, parsed_peak_end),
                    },
                )
                ...
                if cytoband_chromosome != parsed_chromosome:
                    self.logger.error(
                        ...
                        extra={
                            "line_number": self.line_number,
                            "cause": "(%s%s, %s)"
                            % (cytoband_chromosome, parsed_cytoband, parsed_chromosome),
                        },
                    )
    ...
    def parse_amp_value(self, value, column_number):
        ...
        expected_value = str(
            int(self.meta_dict["genetic_alteration_type"] == "GISTIC_GENES_AMP")
        )
        if value != expected_value:
            self.logger.error(
                "'amp' column must be '%s' in files of genetic " "alteration type '%s'",
                expected_value,
                self.meta_dict["genetic_alteration_type"],
                extra={
                    "line_number": self.line_number,
    ...
    def parse_q_value(self, value, column_number):
        ...
        if not value_invalid and (not 0 <= parsed_value <= 1):
            self.logger.error(
                "q-value is not between 0 and 1",
                extra={
                    "line_number": self.line_number,
                    "column_number": column_number,
                    "cause": value,
...
class MultipleDataFileValidator(FeaturewiseFileValidator, metaclass=ABCMeta):
    ...
    def checkHeader(self, cols):
        ...
        if self.get_prior_validated_header() is not None:
            if self.cols != self.get_prior_validated_header():
                self.logger.error(
                    "Headers from data files are different",
                    extra={"line_number": self.line_number},
                )
                num_errors += 1
        else:
    ...
    def parseFeatureColumns(self, nonsample_col_vals):
        ...
        if feature_id == "":
            ...
            self.logger.error(
                "Feature id contains one or more illegal characters",
                extra={
                    "line_number": self.line_number,
                    "cause": "id was`"
                    + feature_id
                    + "` and only alpha-numeric, _, . and - are allowed.",
                },
            )
        else:
            ...
            if self.get_prior_validated_feature_ids() is not None:
                ...
                if feature_id not in self.get_prior_validated_feature_ids():
                    self.logger.error(
                        "Feature id cannot be found in other data file",
                        extra={"line_number": self.line_number, "cause": feature_id},
                    )
...
class GsvaScoreValidator(GsvaWiseFileValidator):
    ...
    def checkValue(self, value, col_index):
        ...
        if stripped_value < -1 or stripped_value > 1:
            self.logger.error(
                "Value is not between -1 and 1, and therefore not "
                "a valid GSVA score",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
...
class GsvaPvalueValidator(GsvaWiseFileValidator):
    ...
    def checkValue(self, value, col_index):
        ...
        if stripped_value <= 0 or stripped_value > 1:
            self.logger.error(
                "Value is not between 0 and 1, and therefore not a valid p-value",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
...
class GenericAssayWiseFileValidator(FeaturewiseFileValidator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.REQUIRED_HEADERS.extend(
            [
                x.strip()
                for x in self.meta_dict["generic_entity_meta_properties"].split(",")
            ]
        )
    ...
    def checkId(self):
        ...
        is_patient_level = (
            True
            if "patient_level" in self.meta_dict
            and self.meta_dict["patient_level"].strip().lower() == "true"
            else False
        )
        ...
        for index, id in enumerate(ids):
            if " " in id:
                ...
                if not self.checkPatientId(
                    id, column_number=self.num_nonsample_cols + index + 1
                ):
                    num_errors += 1
            else:
...
class GenericAssayContinuousValidator(GenericAssayWiseFileValidator):
    ...
    def checkValue(self, value, col_index):
        ...
        try:
            ...
            self.logger.error(
                "Value cannot be interpreted as a floating point number and is not valid value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if math.isnan(numeric_value):
            self.logger.error(
                "Value is NaN, therefore, not a valid value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if math.isinf(numeric_value):
            self.logger.error(
                "Value is infinite and, therefore, not a valid value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if numeric_value % 1 == 0 and not hasTruncSymbol:
            self.logger.warning(
                "Value has no decimals and may represent an invalid value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "generic_assay_type" in self.meta_dict:
            ...
            if generic_assay_type in self.GENERIC_ASSAY_CATEGORICAL_VALUES_MAP_BY_TYPE:
                self.ALLOWED_VALUES = self.GENERIC_ASSAY_CATEGORICAL_VALUES_MAP_BY_TYPE[
                    generic_assay_type
                ]
...
class GenericAssayBinaryValidator(GenericAssayWiseFileValidator):
    ...
    ALLOWED_VALUES = [
        ...
        "true",
        "false",
    ] + GenericAssayWiseFileValidator.NULL_VALUES
...
def process_metadata_files(
    directory, portal_instance, logger, relaxed_mode, strict_maf_checks
):
    ...
    filenames = [
        ...
        and not f.startswith("ONCOKB_IMPORT_BACKUP")
    ...
    reference_genome_map = {
        ...
        "felCat9": ("cat", "Felis_catus_9", "felCat9"),
    }
    ...
    for filename in filenames:
        ...
        if (
            portal_instance.geneset_version is not None
            and "geneset_def_version" in meta_dictionary
        ):
            ...
            if geneset_def_version != portal_instance.geneset_version:
                logger.error(
                    ...
                    extra={"filename_": filename, "cause": geneset_def_version},
                )
        ...
        if "stable_id" in meta_dictionary:
            ...
            if stable_id == "" or re.search(INVALID_ID_CHARACTERS, stable_id) != None:
                logger.error(
                    ...
                    extra={"filename_": filename, "cause": stable_id},
                )
        ...
        if "datatype" in meta_dictionary:
            if meta_dictionary["datatype"].lower() in study_data_types:
                ...
                if meta_dictionary["datatype"].lower() == "seg":
                    logger.error(
                        "datatype SEG is repeated. Only one segmentation file can be loaded."
                    )
            else:
        ...
        if meta_file_type == cbioportal_common.MetaFileTypes.STUDY:
            if study_cancer_type is not None:
                ...
                if (
                    "add_global_case_list" in meta_dictionary
                    and meta_dictionary["add_global_case_list"].lower() == "true"
                ):
            ...
            if "reference_genome" in meta_dictionary:
                if meta_dictionary["reference_genome"] not in reference_genome_map:
                    logger.error(
                        "Unknown reference genome defined. Should be one of %s"
                    ...
                    genome_info = reference_genome_map[
                        meta_dictionary["reference_genome"]
                    ]
                    logger.info(
                        "Setting reference genome to %s (%s, %s)" % genome_info,
                        extra={"filename_": filename},
                    )
                    (
                        portal_instance.species,
                        portal_instance.ncbi_build,
                        portal_instance.reference_genome,
                    ) = genome_info
            else:
                logger.info(
                    "No reference genome specified -- using default (hg19)",
                    extra={"filename_": filename},
                )
            ...
            if "pmid" in meta_dictionary:
                if "citation" not in meta_dictionary:
                    logger.warning(
                        "Citation is required when providing a PubMed ID (pmid)"
                    )
                stripped_pmid_value_len = len(meta_dictionary["pmid"].strip())
                no_whitespace_pmid_value_len = len(
                    "".join(meta_dictionary["pmid"].split())
                )
                if no_whitespace_pmid_value_len != stripped_pmid_value_len:
                    logger.error(
                        "The PMID field in meta_study should not contain any embedded whitespace",
                        extra={"cause": meta_dictionary["pmid"].strip()},
                    )
                for pmid_entry in [
                    x.strip() for x in meta_dictionary["pmid"].split(",")
                ]:
                    try:
                        value = int(pmid_entry)
                    except ValueError:
                        logger.error(
                            "The PMID field in meta_study should be a comma separated list of integers",
                            extra={"cause": pmid_entry},
                        )

            # Validate Study Tags file field in meta_study
            if "tags_file" in meta_dictionary:
                logger.debug("Study Tag file found. It will be validated.")
        ...
        if (
            "data_filename" in meta_dictionary
            and "data_filename" in cbioportal_common.META_FIELD_MAP[meta_file_type]
        ):
    ...
    if study_id is not None:
        for suffix in case_list_suffix_fns:
            defined_case_list_fns[study_id + "_" + suffix] = case_list_suffix_fns[
                suffix
            ]
...
def processCaseListDirectory(
    caseListDir, cancerStudyId, logger, prev_stableid_files=None
):
    ...
    case_list_fns = [
        os.path.join(caseListDir, fn)
        for fn in os.listdir(caseListDir)
        if not (fn.startswith(".") or fn.endswith("~"))
    ]
    ...
    for case in case_list_fns:
        ...
        if not stable_id.startswith(cancerStudyId + "_"):
            logger.error(
                "Stable_id of case list does not start with the "
        ...
        # check for duplicated stable ids
            ...
            logger.error(
                "Multiple case lists with this stable_id defined " "in the study",
                extra={
                    "filename_": case,
                    ...
                    % (
                        stable_id,
                        os.path.relpath(
                            stableid_files[stable_id], os.path.dirname(caseListDir)
                        ),
                    ),
                },
            )
        ...
        if "case_list_category" in meta_dictionary:
            ...
            if (
                meta_dictionary["case_list_category"] in previous_case_list_categories
                and not meta_dictionary["case_list_category"] == "other"
            ):
                logger.warning(
                    "Case list category already used in other case list. Both will be loaded",
                    extra={
                        "filename_": case,
                        "cause": meta_dictionary["case_list_category"],
                    },
                ...
                previous_case_list_categories.append(
                    meta_dictionary["case_list_category"]
                )
        ...
        if len(dupl_sample_ids) > 0:
            logger.warning(
                "Duplicate Sample ID in case list",
                extra={"filename_": case, "cause": ", ".join(dupl_sample_ids.keys())},
            )
        ...
        for value in seen_sample_ids:
            ...
            if value not in DEFINED_SAMPLE_IDS:
                logger.error(
                    "Sample ID not defined in clinical file",
                    extra={"filename_": case, "cause": value},
                )
        ...
        if stable_id.split("_")[-1] == "sequenced":
            ...
            if len(mutation_file_sample_ids) > 0:
                if not mutation_file_sample_ids.issubset(mutation_sample_ids):
                    mutation_sample_ids_not_in_case_list = ", ".join(
                        mutation_file_sample_ids - mutation_sample_ids
                    )
                    logger.error(
...
def validateStudyTags(tags_file_path, logger):
    ...
    logger.debug(
        "Starting validation of study tags file", extra={"filename_": tags_file_path}
    )
    with open(tags_file_path, "r") as stream:
        try:
            parsedYaml = yaml.load(stream, Loader=yaml.FullLoader)
            logger.info(
                "Validation of study tags file complete.",
                extra={"filename_": tags_file_path},
            )
        except yaml.YAMLError:
            logger.error(
                "The file provided is not in YAML or JSON format",
                extra={"filename_": tags_file_path},
            )
...
def validate_data_relations(validators_by_meta_type, logger):
    ...
    for expression_zscores_source_stable_id in expression_zscores_source_stable_ids:
        ...
        if expression_zscores_source_stable_id not in expression_stable_ids.keys():
            logger.error(
                "Invalid source_stable_id. Expected one of ['"
                + "', '".join(expression_stable_ids.keys())
                + "'], which are stable ids of expression files in this study",
                extra={
                    "filename_": expression_zscores_source_stable_ids[
                        expression_zscores_source_stable_id
                    ],
        ...
        for expression_stable_id in expression_stable_ids.keys():
            if (
                ...
                == expression_stable_ids[expression_stable_id]
            ):
    ...
    if any(
        m in validators_by_meta_type for m in ["meta_gsva_pvalues", "meta_gsva_scores"]
    ):
        # When missing a gsva file, no subsequent validation will be done
        missing_gsva_file = False

        # check if both files are present
        if "meta_gsva_pvalues" not in validators_by_meta_type:
            logger.error("Required meta GSVA p-value file is missing")
            missing_gsva_file = True
        if "meta_gsva_scores" not in validators_by_meta_type:
            logger.error("Required meta GSVA score file is missing")
            missing_gsva_file = True
        if "meta_expression" not in validators_by_meta_type:
        ...
        if not missing_gsva_file:
            ...
            if gsva_scores_source_stable_id not in expression_stable_ids.keys():
                logger.error(
                    "Invalid source_stable_id. Expected one of ['"
                    + "', '".join(expression_stable_ids.keys())
                    + "'], which are stable ids of expression files in this study",
                    extra={
                        "filename_": gsva_scores_filename,
            ...
            if len(expression_zscores_source_stable_ids) == 0:
                ...
                if gsva_scores_source_stable_id not in list(
                    expression_zscores_source_stable_ids.keys()
                ):
                    logger.error(
                        ...
                        + "'. Current Z-Score source stable ids found are ['"
...
def request_from_portal_api(server_url, api_name, logger):
    ...
    logger.debug("Requesting %s from portal at '%s'", api_name, server_url)
...
def read_portal_json_file(dir_path, api_name, logger):
    ...
    """
    parsed_json = None
    json_fn = os.path.join(dir_path, "{}.json".format(api_name.replace("/", "_")))
    if os.path.isfile(json_fn):
        logger.debug("Reading portal information from %s", json_fn)
...
def index_api_data(parsed_json, id_field):
    ...
    for attr in parsed_json:
        ...
        # remove id field:
        if id_field not in attr:
            raise RuntimeError("Field '{}' not found in json object".format(id_field))
        id_val = attr[id_field]
        if id_val in transformed_dict:
            raise RuntimeError(
                "Identifier '{}' found more than once in json " "object".format(id_val)
            )
...
def transform_symbol_entrez_map(
    json_data, id_field="hugoGeneSymbol", values_field="entrezGeneId"
):
    ...
    for data_item in json_data:
        ...
        if symbol not in result_dict:
            result_dict[symbol] = []
        result_dict[symbol].append(data_item[values_field])
    return result_dict
...
def load_portal_info(path, logger, offline=False):
    ...
    for api_name, transform_function in (
        ("info", lambda json_data: load_portal_metadata(json_data)),
        ("cancer-types", lambda json_data: index_api_data(json_data, "cancerTypeId")),
        (
            "genes",
            lambda json_data: transform_symbol_entrez_map(json_data, "hugoGeneSymbol"),
        ),
        ("genesets", lambda json_data: extract_ids(json_data, "genesetId")),
        ("genesets_version", lambda json_data: str(json_data).strip(" '[]")),
        ("gene-panels", lambda json_data: extract_panels(json_data, "genePanelId")),
        (
            "genesaliases",
            lambda json_data: transform_symbol_entrez_map(
                json_data, id_field="alias", values_field="entrezGeneId"
            ),
        ),
    ):
        if offline:
    ...
    return PortalInstance(
        portal_info_dict=portal_dict["info"],
...
def interface(args=None):
    ...
    parser.add_argument(
        "-s", "--study_directory", type=str, required=True, help="path to directory."
    )
    portal_mode_group = parser.add_mutually_exclusive_group()
    portal_mode_group.add_argument(
        "-u",
        "--url_server",
        type=str,
    ...
    portal_mode_group.add_argument(
        ...
        "--no_portal_checks",
        action="store_true",
        help="Skip tests requiring information " "from the cBioPortal installation",
    )
    parser.add_argument(
        "-html",
        "--html_table",
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="report status info messages in addition " "to errors and warnings",
    )
    parser.add_argument(
        "-r",
...
def validate_study(study_dir, portal_instance, logger, relaxed_mode, strict_maf_checks):
    ...
    if (
        portal_instance.hugo_entrez_map is None
        or portal_instance.alias_entrez_map is None
    ):
        logger.warning(
            "Skipping validations relating to gene identifiers and "
            "aliases defined in the portal"
        )
    if portal_instance.gene_set_list is None or portal_instance.geneset_version is None:
    ...
    if cbioportal_common.MetaFileTypes.CANCER_TYPE in validators_by_meta_type:
        ...
        if len(cancer_type_validators) > 1:
            logger.error(
                ...
                extra={
                    "cause": ", ".join(
                        ...
                        for validator in validators_by_meta_type[
                            cbioportal_common.MetaFileTypes.CANCER_TYPE
                        ]
                    )
                },
            ...
            studydefined_cancer_types = cancer_type_validators[0].defined_cancer_types
    ...
    if portal_instance.cancer_type_dict is not None and not (
        study_cancer_type in portal_instance.cancer_type_dict
        or study_cancer_type in DEFINED_CANCER_TYPES
    ):
        logger.error(
    ...
    if cbioportal_common.MetaFileTypes.SAMPLE_ATTRIBUTES not in validators_by_meta_type:
        logger.error("No sample attribute file detected")
        return
    if (
        len(validators_by_meta_type[cbioportal_common.MetaFileTypes.SAMPLE_ATTRIBUTES])
        > 1
    ):
        logger.error(
    ...
    if defined_sample_ids is None:
        logger.error(
            "Sample file could not be parsed. Please fix "
    ...
    if cbioportal_common.MetaFileTypes.RESOURCES_DEFINITION in validators_by_meta_type:
        ...
        RESOURCE_DEFINITION_DICTIONARY = (
            resources_definition_validator.resource_definition_dictionary
        )
    ...
    if not os.path.isdir(case_list_dirname):
        logger.info(
            "No directory named 'case_lists' found, so assuming no custom case lists."
        )
    else:
    ...
    validate_defined_caselists(
        study_id,
        list(defined_case_list_fns.keys()),
    ...
    if cbioportal_common.MetaFileTypes.GENE_PANEL_MATRIX in validators_by_meta_type:
        if (
            ...
            validators_by_meta_type[cbioportal_common.MetaFileTypes.GENE_PANEL_MATRIX][
                0
            ].validate()
...
def main_validate(args):
    ...
    collapsing_text_handler = cbioportal_common.CollapsingLogMessageHandler(
        capacity=5e5, flushLevel=logging.CRITICAL, target=text_handler
    )
    ...
    if html_output_filename:
        ...
        html_handler = Jinja2HtmlHandler(study_dir, html_output_filename, capacity=1e5)
        ...
        collapsing_html_handler = cbioportal_common.CollapsingLogMessageHandler(
            capacity=5e5, flushLevel=logging.CRITICAL, target=html_handler
        )
    ...
    if args.error_file:
        ...
        coll_errfile_handler = cbioportal_common.CollapsingLogMessageHandler(
            capacity=5e5, flushLevel=logging.CRITICAL, target=errfile_handler
        )
    ...
    if args.no_portal_checks:
        portal_instance = PortalInstance(
            portal_info_dict=None,
        ...
        portal_instance = load_portal_info(args.portal_info_dir, logger, offline=True)
    else:
...
if __name__ == "__main__":
    ...
    print(
        (
            "Validation of study {status}.".format(
                status={
                    0: "succeeded",
                    1: "failed",
